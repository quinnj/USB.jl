...
function Base.show(io::IO, e::Endpoint, tabs=0)
    x = e.desc
    println(io, "\t"^tabs * "USB.Endpoint(address = $(bits(x.bEndpointAddress)), type = $(Int(x.bDescriptorType)))")
    return
end
...
function Base.show(io::IO, a::AlternateSetting, tabs=0)
    x = a.desc
    println(io, "\t"^tabs * "USB.AlternateSetting(id = $(Int(x.bInterfaceNumber)), type = $(Int(x.bDescriptorType)), class = $(libusb_class_code(x.bInterfaceClass)))")
...
function Base.show(io::IO, c::Configuration, tabs=0)
    x = c.desc
    println(io, "\t"^tabs * "USB.Configuration(id = $(Int(x.bConfigurationValue)), type = $(Int(x.bDescriptorType)))")
...
function devices()
    ...
    for d in list
        ...
        for c = 0x00:desc1[].bNumConfigurations-1
            @show c
            desc2 = Ref{USB.libusb_config_descriptor}(USB.libusb_config_descriptor())
            USB.libusb_get_config_descriptor(d, c, desc2)
            @show desc2[]
            @show desc2[].bNumInterfaces
            interfaces = USB.Interface[]
            push!(configs, USB.Configuration(desc2[], interfaces))
            if desc2[].interface != C_NULL
                inters = unsafe_wrap(Vector{USB.libusb_interface}, desc2[].interface, desc2[].bNumInterfaces)
                for i = 0x01:desc2[].bNumInterfaces
                    @show i
                    inter = inters[i]
